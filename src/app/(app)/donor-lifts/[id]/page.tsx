...
const SL:Record<string,string>={pending_intake:'Pending',ready_for_teardown:'Ready',in_teardown:'Teardown',mostly_complete:'Nearly Done',finished:'Finished',scrap_only:'Scrap'}
const SC:Record<string,string>={pending_intake:'#3b82f6',ready_for_teardown:amb,in_teardown:amb,mostly_complete:'#22c55e',finished:dim,scrap_only:'#ef4444'}
...
export default function Page({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const {data:l} = await (sb.from('donor_lift') as any).select('*').eq('id',params.id).single()
      const {data:p} = await (sb.from('part') as any).select('*').eq('donor_lift_id',params.id).order('created_at',{ascending:false})
      setLift(l)
      setParts(p||[])
      setLoading(false)
    }
  ...
  if (!lift) return <div style={{minHeight:'100dvh',background:bg,display:'flex',alignItems:'center',justifyContent:'center',color:dim}}>Lift not found.</div>
  ...
  const sc = SC[lift.status]||dim
  const sl = SL[lift.status]||lift.status
  ...
  return (
    <div style={{minHeight:'100dvh',background:bg,paddingBottom:'80px'}}>
      <div style={{display:'flex',alignItems:'center',gap:'12px',padding:'16px',background:'#111520',borderBottom:bdr,position:'sticky',top:0,zIndex:30}}>
        ...
        <div style={{padding:'4px 10px',borderRadius:'4px',background:sc+'25',color:sc,fontSize:'10px',fontWeight:700,textTransform:'uppercase',letterSpacing:'1px'}}>{sl}</div>
      </div>
              ...
              return(
                <a key={p.id} href={'/parts/'+p.id} style={{display:'flex',alignItems:'center',gap:'12px',padding:'13px 14px',borderTop:i>0?bdr:'none',textDecoration:'none'}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{fontSize:'11px',color:dim,marginTop:'2px'}}>{p.status?.replace(/_/g,' ')}</div>
                  </div>
