        ...
        {parts.length === 0 ? (
          ...
          <div style={{background:bgMid,border:bdr,borderRadius:'8px',overflow:'hidden'}}>
            {parts.map((p:any,i:number)=>{
              const gradeColor = {A_tested_working:'#22c55e',B_takeout_untested:amb,C_for_parts_or_repair:'#f97316',D_core_only:'#ef4444'}[p.condition_grade]||dim
              const gradeLabel = {A_tested_working:'A',B_takeout_untested:'B',C_for_parts_or_repair:'C',D_core_only:'D'}[p.condition_grade]||'?'
              return (
